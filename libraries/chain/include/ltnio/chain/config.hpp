...
namespace ltnio { namespace chain { namespace config {
...
const static uint64_t system_account_name    = N(ltnio);
const static uint64_t null_account_name      = N(ltnio.null);
const static uint64_t producers_account_name = N(ltnio.prods);
...
const static uint64_t ltnio_auth_scope       = N(ltnio.auth);
const static uint64_t ltnio_all_scope        = N(ltnio.all);

const static uint64_t active_name = N(active);
const static uint64_t owner_name  = N(owner);
const static uint64_t ltnio_any_name = N(ltnio.any);
const static uint64_t ltnio_code_name = N(ltnio.code);
...
const static ltnio::chain::wasm_interface::vm_type default_wasm_runtime = ltnio::chain::wasm_interface::vm_type::wabt;
...
} } } // namespace ltnio::chain::config

constexpr uint64_t LTN_PERCENT(uint64_t value, uint32_t percentage) {
   return (value * percentage) / ltnio::chain::config::percent_100;
}

template<typename Number>
Number LTN_PERCENT_CEIL(Number value, uint32_t percentage) {
   return ((value * percentage) + ltnio::chain::config::percent_100 - ltnio::chain::config::percent_1)  / ltnio::chain::config::percent_100;
}
