...
#define N(X) ltnio::chain::string_to_name(#X)
...
namespace std {
   template<> struct hash<ltnio::chain::name> : private hash<uint64_t> {
      typedef ltnio::chain::name argument_type;
...
namespace fc {
  class variant;
  void to_variant(const ltnio::chain::name& c, fc::variant& v);
  void from_variant(const fc::variant& v, ltnio::chain::name& check);
} // fc


FC_REFLECT( ltnio::chain::name, (value) )
