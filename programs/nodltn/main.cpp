/**
 *  @file
 *  @copyright defined in ltnio/LICENSE.txt
 */
...
{
   try {
      app().set_version(ltnio::nodltn::config::version);

      auto root = fc::app_path();
      app().set_default_data_dir(root / "ltnio" / nodltn::config::node_executable_name / "data" );
      app().set_default_config_dir(root / "ltnio" / nodltn::config::node_executable_name / "config" );
      ...
      initialize_logging();
      ilog("${name} version ${ver}", ("name", nodltn::config::node_executable_name)("ver", app().version_string()));
      ilog("${name} using configuration file ${c}", ("name", nodltn::config::node_executable_name)("c", app().full_config_file_path().string()));
      ilog("${name} data directory is ${d}", ("name", nodltn::config::node_executable_name)("d", app().data_dir().string()));
      app().startup();
      app().exec();
   ...
   ilog("${name} successfully exiting", ("name", nodltn::config::node_executable_name));
   return SUCCESS;
}
