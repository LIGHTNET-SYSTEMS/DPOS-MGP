...
class Cluster(object):
    ...
    __BiosPort=8788
    __LauncherCmdArr=[]
    __bootlog="ltnio-ignition-wd/bootlog.txt"

    # pylint: disable=too-many-arguments
    # walletd [True|False] Is kltnd running. If not load the wallet plugin
    def __init__(self, walletd=False, localCluster=True, host="localhost", port=8888, walletHost="localhost", walletPort=9899, enableMongo=False
                 , mongoHost="localhost", mongoPort=27017, mongoDb="LTNtest", defproduceraPrvtKey=None, defproducerbPrvtKey=None, staging=False):
        """Cluster container.
        walletd [True|False] Is wallet kltnd running. If not load the wallet plugin
        localCluster [True|False] Is cluster local to host.
        host: ltn server host
        port: ltn server port
        walletHost: ltn wallet host
        walletPort: wos wallet port
        enableMongo: Include mongoDb support, configures ltn mongo plugin
        ...
        self.ltnioAccount=self.defProducerAccounts["ltnio"]= Account("ltnio")
    ...
    def launch(self, pnodes=1, unstartedNodes=0, totalNodes=1, prodCount=1, topo="mesh", delay=1, onlyBios=False, dontBootstrap=False,
               totalProducers=None, sharedProducers=0, extraNodltnArgs=None, useBiosBootFile=True, specificExtraNodltnArgs=None, onlySetProds=False,
        ...
        extraNodltnArgs: string of arguments to pass through to each nodoes instance (via --nodltn flag on launcher)
        ...
        specificExtraNodltnArgs: dictionary of arguments to pass to a specific node (via --specific-num and
                                 --specific-nodltn flags on launcher), example: { "5" : "--plugin ltnio::test_control_api_plugin" }
        ...
        loadSystemContract: indicate whether the ltnio.system contract should be loaded (setting this to False causes useBiosBootFile to be treated as False)
        """
        ...
        if not self.localCluster:
            Utils.Print("WARNING: Cluster not local, not launching %s." % (Utils.LtnServerName))
            return True
        ...
        while not Utils.arePortsAvailable(set(range(self.port, self.port+totalNodes+1))):
            Utils.Print("ERROR: Another process is listening on nodltn default port. wait...")
            if tries == 0:
                return False
            tries = tries - 1
            time.sleep(2)
        ...
        cmd="%s -p %s -n %s -d %s -i %s -f %s --unstarted-nodes %s" % (
            Utils.LtnLauncherPath, pnodes, totalNodes, delay, datetime.datetime.utcnow().strftime("%Y-%m-%dT%H:%M:%S.%f")[:-3],
            producerFlag, unstartedNodes)
        cmdArr=cmd.split()
        if self.staging:
            cmdArr.append("--nogen")

        nodltnArgs="--max-transaction-time -1 --abi-serializer-max-time-ms 990000 --filter-on \"*\" --p2p-max-nodes-per-host %d" % (totalNodes)
        if not self.walletd:
            nodltnArgs += " --plugin ltnio::wallet_api_plugin"
        if self.enableMongo:
            nodltnArgs += " --plugin ltnio::mongo_db_plugin --mongodb-wipe --delete-all-blocks --mongodb-uri %s" % self.mongoUri
        if extraNodltnArgs is not None:
            assert(isinstance(extraNodltnArgs, str))
            nodltnArgs += extraNodltnArgs
        if Utils.Debug:
            nodltnArgs += " --contracts-console"
        if PFSetupPolicy.hasPreactivateFeature(pfSetupPolicy):
            nodltnArgs += " --plugin ltnio::producer_api_plugin"

        if nodltnArgs:
            cmdArr.append("--nodltn")
            cmdArr.append(nodltnArgs)

        if specificExtraNodltnArgs is not None:
            assert(isinstance(specificExtraNodltnArgs, dict))
            for nodeNum,arg in specificExtraNodltnArgs.items():
                assert(isinstance(nodeNum, (str,int)))
                assert(isinstance(arg, str))
                cmdArr.append("--specific-num")
                cmdArr.append(str(nodeNum))
                cmdArr.append("--specific-nodltn")
                cmdArr.append(arg)
        ...
        if associatedNodeLabels is not None:
            for nodeNum,label in associatedNodeLabels.items():
                ...
                cmdArr.append(str(nodeNum))
                cmdArr.append("--spcfc-inst-nodltn")
                cmdArr.append(path)
        ...
        if nodes is None or startedNodes != len(nodes):
            Utils.Print("ERROR: Unable to validate %s instances, expected: %d, actual: %d" %
                          (Utils.LtnServerName, startedNodes, len(nodes)))
            return False
        ...
        self.ltnioAccount=self.defProducerAccounts["ltnio"]
    ...
    def getClientVersion(verbose=False):
        ...
        try:
            cmd="%s version client" % (Utils.LtnClientPath)
    ...
    def createAccountKeys(count):
        ...
        for _ in range(0, count):
            try:
                cmd="%s create key --to-console" % (Utils.LtnClientPath)
    ...
    def spreadFunds(self, source, accounts, amount=1):
        ...
        fromm=source
        to=accounts[0]
        Utils.Print("Transfer %s units from account %s to %s on ltn server port %d" % (
        ...
        for i in range(0, count):
            ...
            for _ in range(0, count):
                #Utils.Print("nextLtnIdx: %d, n: %d" % (nextLtnIdx, n))
                nextLtnIdx=(nextLtnIdx + 1)%count
                if not self.nodes[nextLtnIdx].killed:
                    #Utils.Print("nextLtnIdx: %d" % (nextLtnIdx))
            ...
            #Utils.Print("nextLtnIdx: %d, count: %d" % (nextLtnIdx, count))
            node=self.nodes[nextLtnIdx]
    ...
    def validateSpreadFunds(self, initialBalances, transferAmount, source, accounts):
        ...
        for node in self.nodes:
            ...
            if Utils.Debug: Utils.Print("Validate funds on %s server port %d." %
                                        (Utils.LtnServerName, node.port))

            if node.validateFunds(initialBalances, transferAmount, source, accounts) is False:
                Utils.Print("ERROR: Failed to validate funds on ltn node port: %d" % (node.port))
                return False
    ...
    def spreadFundsAndValidate(self, transferAmount=1):
        ...
        initialBalances=self.nodes[0].getLtnBalances([self.defproduceraAccount] + self.accounts)
        assert(initialBalances)
        assert(isinstance(initialBalances, dict))
    ...
    def validateAccounts(self, accounts, testSysAccounts=True):
        ...
        myAccounts = []
        if testSysAccounts:
            myAccounts += [self.ltnioAccount, self.defproduceraAccount, self.defproducerbAccount]
        if accounts:
            assert(isinstance(accounts, list))
            myAccounts += accounts
    ...
    def parseClusterKeys(totalNodes):
        ...
        if producerKeys is None:
            Utils.Print("ERROR: Failed to parse ltnio private keys from cluster config files.")
            return None
    ...
    def bios_bootstrap(self, biosNode, totalNodes, pfSetupPolicy, silent=False):
        """Bootstrap cluster using the bios_boot.sh script generated by ltnio-launcher."""
        ...
        if Utils.Debug: Utils.Print("cmd: %s" % (cmd))
        env = {
            "BIOS_CONTRACT_PATH": "unittests/contracts/old_versions/v1.6.0-rc3/ltnio.bios",
            "FEATURE_DIGESTS": ""
        }
        if PFSetupPolicy.hasPreactivateFeature(pfSetupPolicy):
            env["BIOS_CONTRACT_PATH"] = "unittests/contracts/ltnio.bios"
        ...
        if 0 != subprocess.call(cmd.split(), stdout=Utils.FNull, env=env):
            if not silent: Utils.Print("Launcher failed to shut down ltn cluster.")
            return None
        ...
        ltnioName="ltnio"
        ltnioKeys=producerKeys[ltnioName]
        ltnioAccount=Account(ltnioName)
        ltnioAccount.ownerPrivateKey=ltnioKeys["private"]
        ltnioAccount.ownerPublicKey=ltnioKeys["public"]
        ltnioAccount.activePrivateKey=ltnioKeys["private"]
        ltnioAccount.activePublicKey=ltnioKeys["public"]
        producerKeys.pop(ltnioName)

        if not self.walletMgr.importKey(ltnioAccount, ignWallet):
            Utils.Print("ERROR: Failed to import %s account keys into ignition wallet." % (ltnioName))
            return None

        initialFunds="1000000.0000 {0}".format(CORE_SYMBOL)
        Utils.Print("Transfer initial fund %s to individual accounts." % (initialFunds))
        trans=None
        contract="ltnio.token"
        action="transfer"
        for name, keys in producerKeys.items():
            data="{\"from\":\"ltnio\",\"to\":\"%s\",\"quantity\":\"%s\",\"memo\":\"%s\"}" % (name, initialFunds, "init ltnio transfer")
            opts="--permission ltnio@active"
            if name != "ltnio":
                trans=biosNode.pushMessage(contract, action, data, opts)
                if trans is None or not trans[0]:
                    Utils.Print("ERROR: Failed to transfer funds from ltnio.token to %s." % (name))
                    return None
    ...
    def bootstrap(self, biosNode, totalNodes, prodCount, totalProducers, pfSetupPolicy, onlyBios=False, onlySetProds=False, loadSystemContract=True):
        ...
        trans=biosNode.publishContract(ltnioAccount.name, contractDir, wasmFile, abiFile, waitForTransBlock=True)
        if trans is None:
        ...
        for name, keys in producerKeys.items():
            ...
            trans=biosNode.createAccount(initx, ltnioAccount, 0)
            if trans is None:
        ...
        if not onlyBios:
            if prodCount == -1:
                ...
                with open(setProdsFile, "r") as f:
                    ...
                    opts="--permission ltnio@active"
                    myTrans=biosNode.pushMessage("ltnio", "setprods", setProdsStr, opts)
                ...
                trans=biosNode.pushMessage("ltnio", "setprods", setProdsStr, opts)
                if trans is None or not trans[0]:
            ...
            # wait for block production handover (essentially a block produced by anyone but ltnio).
            lam = lambda: biosNode.getInfo(exitOnError=True)["head_block_producer"] != "ltnio"
        ...
        ltnioTokenAccount=copy.deepcopy(ltnioAccount)
        ltnioTokenAccount.name="ltnio.token"
        trans=biosNode.createAccount(ltnioTokenAccount, ltnioAccount, 0)
        if trans is None:
            Utils.Print("ERROR: Failed to create account %s" % (ltnioTokenAccount.name))
            return None

        ltnioRamAccount=copy.deepcopy(ltnioAccount)
        ltnioRamAccount.name="ltnio.ram"
        trans=biosNode.createAccount(ltnioRamAccount, ltnioAccount, 0)
        if trans is None:
            Utils.Print("ERROR: Failed to create account %s" % (ltnioRamAccount.name))
            return None

        ltnioRamfeeAccount=copy.deepcopy(ltnioAccount)
        ltnioRamfeeAccount.name="ltnio.ramfee"
        trans=biosNode.createAccount(ltnioRamfeeAccount, ltnioAccount, 0)
        if trans is None:
            Utils.Print("ERROR: Failed to create account %s" % (ltnioRamfeeAccount.name))
            return None

        ltnioStakeAccount=copy.deepcopy(ltnioAccount)
        ltnioStakeAccount.name="ltnio.stake"
        trans=biosNode.createAccount(ltnioStakeAccount, ltnioAccount, 0)
        if trans is None:
            Utils.Print("ERROR: Failed to create account %s" % (ltnioStakeAccount.name))
            return None
        ...
        abiFile="%s.abi" % (contract)
        Utils.Print("Publish %s contract" % (contract))
        trans=biosNode.publishContract(ltnioTokenAccount.name, contractDir, wasmFile, abiFile, waitForTransBlock=True)
        if trans is None:
            Utils.Print("ERROR: Failed to publish contract %s." % (contract))
            return None

        # Create currency0000, followed by issue currency0000
        contract=ltnioTokenAccount.name
        Utils.Print("push create action to %s contract" % (contract))
        action="create"
        data="{\"issuer\":\"%s\",\"maximum_supply\":\"1000000000.0000 %s\"}" % (ltnioAccount.name, CORE_SYMBOL)
        opts="--permission %s@active" % (contract)
        trans=biosNode.pushMessage(contract, action, data, opts)
        if trans is None or not trans[0]:
            Utils.Print("ERROR: Failed to push create action to ltnio contract.")
            return None
        ...
        action="issue"
        data="{\"to\":\"%s\",\"quantity\":\"1000000000.0000 %s\",\"memo\":\"initial issue\"}" % (ltnioAccount.name, CORE_SYMBOL)
        opts="--permission %s@active" % (ltnioAccount.name)
        trans=biosNode.pushMessage(contract, action, data, opts)
        if trans is None or not trans[0]:
            Utils.Print("ERROR: Failed to push issue action to ltnio contract.")
            return None
        ...
        Utils.Print("Verify ltnio issue, Expected: %s" % (expectedAmount))
        actualAmount=biosNode.getAccountLtnBalanceStr(ltnioAccount.name)
        ...
        action="transfer"
        for name, keys in producerKeys.items():
            data="{\"from\":\"%s\",\"to\":\"%s\",\"quantity\":\"%s\",\"memo\":\"%s\"}" % (ltnioAccount.name, name, initialFunds, "init transfer")
            opts="--permission %s@active" % (ltnioAccount.name)
            trans=biosNode.pushMessage(contract, action, data, opts)
            if trans is None or not trans[0]:
                Utils.Print("ERROR: Failed to transfer funds from %s to %s." % (ltnioTokenAccount.name, name))
                return None
        ...
        if loadSystemContract:
            ...
            opts="--permission %s@active" % (ltnioAccount.name)
            trans=biosNode.pushMessage(ltnioAccount.name, action, data, opts)
    ...
    @staticmethod
    def pgrepLtnServers(timeout=None):
        cmd=Utils.pgrepCmd(Utils.LtnServerName)
    ...
    @staticmethod
    def pgrepLtnServerPattern(nodeInstance):
        dataLocation=Utils.getNodeDataDir(nodeInstance)
        return r"[\n]?(\d+) (.* --data-dir %s .*)\n" % (dataLocation)

    # Populates list of LtnInstanceInfo objects, matched to actual running instances
    def discoverLocalNodes(self, totalNodes, timeout=None):
        nodes=[]

        psOut=Cluster.pgrepLtnServers(timeout)
        if psOut is None:
    ...
    def discoverLocalNode(self, nodeNum, psOut=None, timeout=None):
        ...
        if psOut is None:
            Utils.Print("ERROR: No nodes discovered.")
            return None
        pattern=Cluster.pgrepLtnServerPattern(nodeNum)
        m=re.search(pattern, psOut, re.MULTILINE)
        if m is None:
            Utils.Print("ERROR: Failed to find %s pid. Pattern %s" % (Utils.LtnServerName, pattern))
            return None
    ...
    def discoverBiosNode(self, timeout=None):
        psOut=Cluster.pgrepLtnServers(timeout=timeout)
        pattern=Cluster.pgrepLtnServerPattern("bios")
        Utils.Print("pattern={\n%s\n}, psOut=\n%s\n" % (pattern,psOut))
        m=re.search(pattern, psOut, re.MULTILINE)
        if m is None:
            Utils.Print("ERROR: Failed to find %s pid. Pattern %s" % (Utils.LtnServerName, pattern))
    ...
    # Kills a percentange of Ltn instances starting from the tail and update ltnInstanceInfos state
    def killSomeLtnInstances(self, killCount, killSignalStr=Utils.SigKillTag):
        killSignal=signal.SIGKILL
        if killSignalStr == Utils.SigTermTag:
            killSignal=signal.SIGTERM
        Utils.Print("Kill %d %s instances with signal %s." % (killCount, Utils.LtnServerName, killSignal))
    ...
    def relaunchLtnInstances(self, cachePopen=False):
    ...
    def killall(self, silent=True, allInstances=False):
        """Kill cluster nodltn instances. allInstances will kill all nodltn instances running on the system."""
        cmd="%s -k 9" % (Utils.LtnLauncherPath)
        if Utils.Debug: Utils.Print("cmd: %s" % (cmd))
        if 0 != subprocess.call(cmd.split(), stdout=Utils.FNull):
            if not silent: Utils.Print("Launcher failed to shut down ltn cluster.")

        if allInstances:
            # ocassionally the launcher cannot kill the ltn server
            cmd="pkill -9 %s" % (Utils.LtnServerName)
            if Utils.Debug: Utils.Print("cmd: %s" % (cmd))
            if 0 != subprocess.call(cmd.split(), stdout=Utils.FNull):
                if not silent: Utils.Print("Failed to shut down ltn cluster.")
    ...
    def bounce(self, nodes, silent=True):
        """Bounces nodltn instances as indicated by parameter nodes.
    ...
    def down(self, nodes, silent=True):
        """Brings down nodltn instances as indicated by parameter nodes.
        nodes should take the form of a comma-separated list as accepted by the launcher --bounce command (e.g. '00' or '00,01')"""
        cmdArr = Cluster.__LauncherCmdArr.copy()
        cmdArr.append("--down")
        cmdArr.append(nodes)
