...
@test "${TEST_LABEL} > previous install prompt" {
  ...
  [[ -z $(echo "${output}") ]] || exit
  # Exists, prompt
  mkdir -p $LTNIO_INSTALL_DIR
  run previous-install-prompt
  [[ ! -z $(echo "${output}" | grep "LTNIO has already been installed into ${LTNIO_INSTALL_DIR}") ]] || exit
  rm -rf $LTNIO_INSTALL_DIR
}
