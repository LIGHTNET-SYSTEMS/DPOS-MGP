...
SCRIPT_LOCATION="scripts/ltnio_build.sh"
TEST_LABEL="[ltnio_build]"

###################################################################
# ON MAC YOU NEED TO FULLY UNINSTALL LTNIO BEFORE THESE WILL PASS #
###################################################################
...
@test "${TEST_LABEL} > Testing Arguments & Options" {
    ...
    if [[ $NAME =~ "Amazon Linux" ]] || [[ $NAME == "CentOS Linux" ]]; then
        ...
        run bash -c "printf \"y\ny\nn\nn\n\" | ./scripts/ltnio_build.sh"
        [[ ! -z $(echo "${output}" | grep "LTNIO compiler checks require the 'which'") ]] || exit
    fi
    ...
    # lack of -i
    [[ ! -z $(echo "${output}" | grep "LTNIO_INSTALL_DIR: ${HOME}/ltnio/${LTNIO_VERSION}") ]] || exit
    ## -o
    run bash -c "printf \"y\ny\nn\nn\n\" | ./$SCRIPT_LOCATION -o Debug -P"
    [[ ! -z $(echo "${output}" | grep "CMAKE_BUILD_TYPE: Debug") ]] || exit
    ## -s
    run bash -c "printf \"y\ny\nn\nn\n\" | ./$SCRIPT_LOCATION -s LTN2 -P"
    [[ ! -z $(echo "${output}" | grep "CORE_SYMBOL_NAME: LTN2") ]] || exit
    ## -b
    run bash -c "printf \"y\ny\nn\nn\n\" | ./$SCRIPT_LOCATION -b /test -P"
    [[ ! -z $(echo "${output}" | grep "BOOST_LOCATION: /test") ]] || exit
    ## -i
    run bash -c "printf \"y\ny\nn\nn\n\"| ./$SCRIPT_LOCATION -i /NEWPATH -P"
    [[ ! -z $(echo "${output}" | grep "LTNIO_INSTALL_DIR: /NEWPATH") ]] || exit
    [[ ! -z $(echo "${output}" | grep "TEMP_DIR: ${HOME}/tmp") ]] || exit
        ...
        [[ ! -z $(echo "${output}" | grep "LTNIO_INSTALL_DIR: $TEMP_DIR/NEWPATH") ]] || exit
