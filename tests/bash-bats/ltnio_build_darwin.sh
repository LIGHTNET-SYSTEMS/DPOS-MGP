...
export SCRIPT_LOCATION="scripts/ltnio_build.sh"
export TEST_LABEL="[ltnio_build_darwin]"
...
## Needed to load ltnio_build_ files properly; it can be empty
@test "${TEST_LABEL} > General" {
    ...
    [[ ! -z $(echo "${output}" | grep "Starting LTNIO Dependency Install") ]] || exit
    ...
    [[ ! -z $(echo "${output}" | grep "brew tap ltnio/ltnio") ]] || exit
    ...
    [[ ! -z $(echo "${output}" | grep "Starting LTNIO Build") ]] || exit
    [[ ! -z $(echo "${output}" | grep " --with-iostreams --with-date_time") ]] || exit # BOOST
    [[ ! -z $(echo "${output}" | grep "LTNIO has been successfully built") ]] || exit
}
