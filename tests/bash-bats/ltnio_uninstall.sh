...
SCRIPT_LOCATION="scripts/ltnio_uninstall.sh"
TEST_LABEL="[ltnio_uninstall]"
...
@test "${TEST_LABEL} > Testing user prompts" {
  ...
  [[ $output =~ " - LTNIO Removal Complete" ]] || exit
  ## First no shows "Cancelled..."
  run bash -c "echo \"n\" | ./$SCRIPT_LOCATION"
  [[ "${output##*$'\n'}" =~ "Cancelled LTNIO Removal!" ]] || exit
...
@test "${TEST_LABEL} > -i" {
  run ./$SCRIPT_LOCATION -y -i ltniotest
  [[ $output =~ .*/ltniotest ]] || exit
  ([[ ! $output =~ "Library/Application\ Support/ltnio" ]] && [[ ! $output =~ ".local/share/ltnio" ]]) || exit
  [[ ! $output =~ "LTNIO Removal Complete" ]] || exit
}
