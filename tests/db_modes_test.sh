...
LTNIO_STUFF_DIR=$(mktemp -d)
trap "rm -rf $LTNIO_STUFF_DIR" EXIT
NODLTN_LAUNCH_PARAMS="./programs/nodltn/nodltn -d $LTNIO_STUFF_DIR --config-dir $LTNIO_STUFF_DIR \
--chain-state-db-size-mb 8 --chain-state-db-guard-size-mb 0 --reversible-blocks-db-size-mb 1 \
--reversible-blocks-db-guard-size-mb 0 -e -pltnio"

run_nodltn() {
   if (( $VERBOSE == 0 )); then
      $NODLTN_LAUNCH_PARAMS --http-server-address '' --p2p-listen-endpoint '' "$@" 2>/dev/null &
   else
      $NODLTN_LAUNCH_PARAMS --http-server-address '' --p2p-listen-endpoint '' "$@" &
   fi
}
...
run_expect_failure() {
   ...
   trap "kill $NODLTN_PID; wait $NODLTN_PID; exit 1" ALRM
   sleep 10
   if wait $NODLTN_PID; then exit 1; fi
   kill $TIMER_PID
   trap ALRM
}
