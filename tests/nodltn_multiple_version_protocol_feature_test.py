...
try:
    ...
    assert cluster.launch(pnodes=4, totalNodes=4, prodCount=1, totalProducers=4,
                          extraNodltnArgs=" --plugin ltnio::producer_api_plugin ",
    ...
    # Finally, when we restart the 4th node with the version of nodltn that supports protocol feature,
    ...
    oldNode.relaunch(oldNodeId, chainArg="--import-reversible-blocks {}".format(portableRevBlkPath), timeout=1, nodltnPath="programs/nodltn/nodltn")
    os.remove(portableRevBlkPath)

    restartNode(oldNode, oldNodeId, chainArg="--replay", nodltnPath="programs/nodltn/nodltn")
