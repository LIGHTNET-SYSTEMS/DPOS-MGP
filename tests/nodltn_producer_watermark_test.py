...
###############################################################
# nodltn_producer_watermark_test
...
def setProds(sharedProdKey):
    ...
    trans=cluster.biosNode.pushMessage("ltnio", "setprods", setProdsStr, opts)
...
try:
    ...
    while tries > 0:
        ...
        info = node.getInfo()
        if node.infoValid and node.lastRetrievedHeadBlockProducer != "ltnio":
            break
        time.sleep(1)
        tries = tries-1
    if tries == 0:
