...
###############################################################
# nodltn_run_remote_test
#  Tests remote capability of the nodltn_run_test. Test will setup cluster and pass nodes info to nodltn_run_test. E.g.
#  nodltn_run_remote_test.py -v --clean-run --dump-error-detail
###############################################################
...
pnodes=1
total_nodes=pnodes
actualTest="tests/nodltn_run_test.py"
...
try:
    ...
    Print("Starting up %s test: %s" % ("nodltn", actualTest))
    Print("cmd: %s\n" % (cmd))
    ...
    TestHelper.shutdown(cluster, None, testSuccessful, killLtnInstances, False, False, killAll, dumpErrorDetails)
