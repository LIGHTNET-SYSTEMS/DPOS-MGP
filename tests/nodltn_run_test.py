...
try:
    ...
    contractDir="unittests/contracts/ltnio.token"
    wasmFile="ltnio.token.wasm"
    abiFile="ltnio.token.abi"
    ...
    if not enableMongo:
        ...
        account=node.getLtnAccountFromDb(currencyAccount.name)
        ...
        if abiName != "account" or abiActionName != "close" or abiType != "close":
            errorExit("FAILURE - get LTN account failed", raw=True)
    ...
    account=node.getLtnAccount(defproduceraAccount.name, exitOnError=True)
    ...
    if not enableMongo:
        ...
        rawAccount=node.getLtnAccount(defproduceraAccount.name, exitOnError=True, returnType=ReturnType.raw)
