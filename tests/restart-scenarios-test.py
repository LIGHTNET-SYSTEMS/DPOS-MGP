...
# --kill-count <nodltn instances to kill>
...
try:
    ...
    Print("Stand up LTN wallet kltnd")
    accountsCount=total_nodes
    walletName="MyWallet"
    Print("Creating wallet %s if one doesn't already exist." % walletName)
    wallet=walletMgr.create(walletName, [cluster.ltnioAccount,cluster.defproduceraAccount,cluster.defproducerbAccount])
    ...
    if cluster.killSomeLtnInstances(killCount, killSignal) is False:
        errorExit("Failed to kill Ltn instances")
    Print("nodltn instances killed.")
    ...
    if cluster.relaunchLtnInstances(cachePopen=True) is False:
        errorExit("Failed to relaunch Ltn instances")
    Print("nodltn instances relaunched.")
    ...
    if killLtnInstances:
        ...
        assert atLeastOne, "Test is setup to verify that a cleanly interrupted nodltn exits with an exit status of 0, but this test may no longer be setup to do that"

    testSuccessful=True
finally:
    TestHelper.shutdown(cluster, walletMgr, testSuccessful=testSuccessful, killLtnInstances=killLtnInstances, killWallet=killLtnInstances, keepLogs=keepLogs, cleanRun=killAll, dumpErrorDetails=dumpErrorDetails)
