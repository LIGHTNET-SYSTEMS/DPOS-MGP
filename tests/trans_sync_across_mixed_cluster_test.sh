...
killAll()
{
  programs/ltnio-launcher/ltnio-launcher -k 15
}

cleanup()
{
    rm -rf etc/ltnio/node_*
    rm -rf var/lib/node_*
}
...
getHeadBlockNum()
{
  INFO="$(programs/clltn/clltn get info)"
  verifyErrorCode "clltn get info"
...
# stand up nodltn cluster
launcherOpts="-p $pnodes -n $total_nodes -s $topo -d $delay"
echo Launcher options: --nodltn \"--plugin ltnio::wallet_api_plugin\" $launcherOpts
programs/ltnio-launcher/ltnio-launcher --nodltn "--plugin ltnio::wallet_api_plugin" $launcherOpts
sleep 7
...
while [ $port2  -ne $endport ]; do
    ...
    TRANS_INFO="$(programs/clltn/clltn --port $port2 get block 1)"
    verifyErrorCode "clltn get block"
    port2=`expr $port2 + 1`
done

# create 3 keys
KEYS="$(programs/clltn/clltn create key)"
verifyErrorCode "clltn create key"
...
PASSWORD_INITA="$(programs/clltn/clltn wallet create --name inita)"
verifyErrorCode "clltn wallet create"
# strip out password from output
PASSWORD_INITA="$(echo "$PASSWORD_INITA" | awk '/PW/ {print $1}')"
# remove leading/trailing quotes
PASSWORD_INITA=${PASSWORD_INITA#\"}
PASSWORD_INITA=${PASSWORD_INITA%\"}
programs/clltn/clltn wallet import --name inita --private-key $INITA_PRV_KEY
verifyErrorCode "clltn wallet import"
programs/clltn/clltn wallet import --name inita --private-key $PRV_KEY1
verifyErrorCode "clltn wallet import"
programs/clltn/clltn wallet import --name inita --private-key $PRV_KEY2
verifyErrorCode "clltn wallet import"
programs/clltn/clltn wallet import --name inita --private-key $PRV_KEY3
verifyErrorCode "clltn wallet import"
...
ACCOUNT_INFO="$(programs/clltn/clltn create account inita testera $PUB_KEY1 $PUB_KEY3)"
verifyErrorCode "clltn create account"
waitForNextBlock
# verify account created
ACCOUNT_INFO="$(programs/clltn/clltn get account testera)"
verifyErrorCode "clltn get account"
...
while [ $port  -ne $endport ]; do
    ...
    TRANSFER_INFO="$(programs/clltn/clltn transfer inita testera 975321 "test transfer")"
    verifyErrorCode "clltn transfer"
   ...
   TRANS_INFO="$(programs/clltn/clltn --port $port2 get transaction $TRANS_ID)"
   verifyErrorCode "clltn get transaction trans_id of <$TRANS_INFO> from node on port $port2"
	port2=`expr $port2 + 1`
    done
