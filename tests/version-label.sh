#!/bin/bash
# The purpose of this test is to ensure that the output of the "nodltn --version" command matches the version string defined by our CMake files
# If the environment variable BUILDKITE_TAG is empty or unset, this test will echo success
echo '##### Nodltn Version Label Test #####'
...
# orient ourselves
[[ "$LTNIO_ROOT" == '' ]] && LTNIO_ROOT=$(echo $(pwd)/ | grep -ioe '.*/ltn/')
[[ "$LTNIO_ROOT" == '' ]] && LTNIO_ROOT=$(echo $(pwd)/ | grep -ioe '.*/LTNIO/ltnio/')
[[ "$LTNIO_ROOT" == '' ]] && LTNIO_ROOT=$(echo $(pwd)/ | grep -ioe '.*/build/' | sed 's,/build/,,')
echo "Using LTNIO_ROOT=\"$LTNIO_ROOT\"."
# determine expected value
CMAKE_CACHE="$LTNIO_ROOT/build/CMakeCache.txt"
CMAKE_LISTS="$LTNIO_ROOT/CMakeLists.txt"
if [[ -f "$CMAKE_CACHE" && $(cat "$CMAKE_CACHE" | grep -c 'DOXY_LTN_VERSION') > 0 ]]; then
    echo "Parsing \"$CMAKE_CACHE\"..."
    EXPECTED="v$(cat "$CMAKE_CACHE" | grep 'DOXY_LTN_VERSION' | cut -d '=' -f 2)"
...
if [[ "$EXPECTED" == '' ]]; then
    ...
    echo ''
    echo '$ cat "$CMAKE_CACHE" | grep "DOXY_LTN_VERSION"'
    cat "$CMAKE_CACHE" | grep "DOXY_LTN_VERSION"
    echo '$ pwd'
    pwd
    echo '$ ls -la "$LTNIO_ROOT"'
    ls -la "$LTNIO_ROOT"
    echo '$ ls -la "$LTNIO_ROOT/build"'
    ls -la "$LTNIO_ROOT/build"
    exit 1
fi
echo "Expecting \"$EXPECTED\"..."
# get nodltn version
ACTUAL=$($LTNIO_ROOT/build/bin/nodltn --version) || : # nodltn currently returns -1 for --version
# test
