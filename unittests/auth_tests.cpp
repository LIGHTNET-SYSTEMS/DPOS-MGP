...
BOOST_AUTO_TEST_CASE(link_auths) { try {
   ...
   // Link authority for ltnio reqauth action with alice's spending key
   chain.link_authority("alice", "ltnio", "spending",  "reqauth");
   ...
   BOOST_CHECK_THROW( chain.link_authority("alice", "ltnio", "spending",  "reqauth"), action_validate_exception);

   // Unlink alice with ltnio reqauth
   chain.unlink_authority("alice", "ltnio", "reqauth");
   ...
   // Link authority for any ltnio action with alice's scud key
   chain.link_authority("alice", "ltnio", "scud");
...
BOOST_AUTO_TEST_CASE(link_then_update_auth) { try {
   ...
   chain.link_authority("alice", "ltnio", "first",  "reqauth");
...
try {
   ...
   // Creating account with ltnio. prefix with privileged account
   chain.create_account("ltnio.test1");

   // Creating account with ltnio. prefix with non-privileged account, should fail
   BOOST_CHECK_EXCEPTION(chain.create_account("ltnio.test2", "joe"), action_validate_exception,
                         fc_exception_message_is("only privileged accounts can have names that start with 'ltnio.'"));
...
BOOST_AUTO_TEST_CASE( any_auth ) { try {
   ...
   chain.link_authority( "alice", "ltnio", "ltnio.any", "reqauth" );
   chain.link_authority( "bob", "ltnio", "ltnio.any", "reqauth" );

   /// this should succeed because ltnio::reqauth is linked to any permission
   chain.push_reqauth("alice", { permission_level{N(alice), "spending"} }, { spending_priv_key });
...
try {
   ...
   using resource_usage_object = ltnio::chain::resource_limits::resource_usage_object;
   using by_owner = ltnio::chain::resource_limits::by_owner;
