...
public:
   ...
   fc::variant get_global_state() {
    ...
    asset get_balance( const account_name& act ) {
         return get_currency_balance(N(ltnio.token), symbol(CORE_SYMBOL), act);
    }
...
BOOST_FIXTURE_TEST_CASE( bootseq_test, bootseq_tester ) {
    try {
        ...
        // Create ltnio.msig and ltnio.token
        create_accounts({N(ltnio.msig), N(ltnio.token), N(ltnio.ram), N(ltnio.ramfee), N(ltnio.stake), N(ltnio.vpay), N(ltnio.bpay), N(ltnio.saving) });
        // Set code for the following accounts:
        //  - ltnio (code: ltnio.bios) (already set by tester constructor)
        //  - ltnio.msig (code: ltnio.msig)
        //  - ltnio.token (code: ltnio.token)
        // set_code_abi(N(ltnio.msig), contracts::ltnio_msig_wasm(), contracts::ltnio_msig_abi().data());//, &ltnio_active_pk);
        // set_code_abi(N(ltnio.token), contracts::ltnio_token_wasm(), contracts::ltnio_token_abi().data()); //, &ltnio_active_pk);

        set_code_abi(N(ltnio.msig),
                     contracts::ltnio_msig_wasm(),
                     contracts::ltnio_msig_abi().data());//, &ltnio_active_pk);
        set_code_abi(N(ltnio.token),
                     contracts::ltnio_token_wasm(),
                     contracts::ltnio_token_abi().data()); //, &ltnio_active_pk);

        // Set privileged for ltnio.msig and ltnio.token
        set_privileged(N(ltnio.msig));
        set_privileged(N(ltnio.token));

        // Verify ltnio.msig and ltnio.token is privileged
        const auto& ltnio_msig_acc = get<account_metadata_object, by_name>(N(ltnio.msig));
        BOOST_TEST(ltnio_msig_acc.is_privileged() == true);
        const auto& ltnio_token_acc = get<account_metadata_object, by_name>(N(ltnio.token));
        BOOST_TEST(ltnio_token_acc.is_privileged() == true);


        // Create SYS tokens in ltnio.token, set its manager as ltnio
        ...
        create_currency(N(ltnio.token), config::system_account_name, max_supply);
        // Issue the genesis supply of 1 billion SYS tokens to ltnio.system
        issue(N(ltnio.token), config::system_account_name, config::system_account_name, initial_supply);
        ...
        for( const auto& a : test_genesis ) {
           ...
           r = delegate_bandwidth(N(ltnio.stake), a.aname, asset(net), asset(cpu));
           BOOST_REQUIRE( !r->except_ptr );
        }
        ...
        BOOST_TEST(active_schedule.producers.front().producer_name == "ltnio");
        ...
        BOOST_REQUIRE_THROW(claim_rewards(N(runnerup1)), ltnio_assert_message_exception);
        ...
        BOOST_REQUIRE_THROW(undelegate_bandwidth(N(b1), N(b1), core_from_string("49999500.0000"), core_from_string("49999500.0000")), ltnio_assert_message_exception);
