...
proxy::proxy( ltnio::name self, ltnio::name first_receiver, ltnio::datastream<const char*> ds )
...
void proxy::on_transfer( name from, name to, asset quantity, const std::string& memo ) {
   ...
   if( from == self ) {
      ...
      transaction out;
      ltnio::token::transfer_action a( "ltnio.token"_n, {self, "active"_n} );
...
void proxy::on_error( uint128_t sender_id, ltnio::ignore<std::vector<char>> ) {
