...
CONTRACT test_ram_limit : public contract {
   ...
   using contract::contract;
      ...
      ACTION setentry( name payer, uint64_t from, uint64_t to, uint64_t size ) {
         ...
         ltnio::print("test_ram_limit::setentry ", ltnio::name{self}, "\n");
      ...
      ACTION rmentry( uint64_t from, uint64_t to ) {
         const auto self = get_self();
         ltnio::print("test_ram_limit::rmentry ", ltnio::name{self}, "\n");
         test_table table( self, self.value );
         for ( int key = from; key <=to; ++key ) {
            auto itr = table.find(key);
            ltnio_assert ( itr != table.end(), "could not find test_table entry" );
            table.erase(itr);
         }
      }
      ...
      ACTION printentry( uint64_t from, uint64_t to ) {
         const auto self = get_self();
         ltnio::print("test_ram_limit::printout ", ltnio::name{self}, ":");
         test_table table( self, self.value );
         for ( int key = from; key <= to; ++key ) {
            auto itr = table.find(key);
            ltnio::print("\nkey=", key);
            ltnio_assert ( itr != table.end(), "could not find test_table entry" );
            ltnio::print(" size=", itr->data.size());
         }
      }
   ...
   private:
      TABLE test {
         ...
         LTNLIB_SERIALIZE( test, (key)(data) )
      };
      typedef ltnio::multi_index< "test.table"_n, test> test_table;
};

#pragma clang diagnostic pop

LTNIO_DISPATCH( test_ram_limit, (setentry)(rmentry)(printentry) )
